...
const requireModule = (module: AppModule) => {
  return (req: Request, res: Response, next: () => void) => {
    requireAuth(req, res, () => {
      ...
      const allowed = req.user?.allowedModules || ['appointments', 'exams', 'chats', 'simulator', 'billing'];
...
apiRouter.post('/api/waha/extract-contacts', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // Função interna para limpar e normalizar telefone brasileiro (somente dígitos válidos)
    ...
    if (source === 'contacts' || source === 'all') {
      ...
      for (const c of contacts) {
        if (c.isGroup || c.id?.includes('@g.us') || c.id?.includes('@broadcast') || c.id?.includes('@lid')) continue;
      ...
      for (const ch of chats) {
        ...
        if (typeof chatId !== 'string' || chatId.includes('@g.us') || chatId.includes('@broadcast') || chatId.includes('@lid')) continue;
    ...
    if (source === 'groups' || source === 'all') {
      ...
      for (const gId of targetGroupIds) {
        ...
        // Passo 2: Se não vieram embutidos, busca via getGroupParticipants
        ...
        for (const p of participants) {
          ...
          if (typeof p === 'string') {
            ...
            rawId = p.pn || p.phoneNumber || p.number || (typeof p.id === 'string' ? p.id : (p.id?.user || p.id?._serialized || p.jid || p.user || ''));
...
/* ==========================================================================
   ROTAS: GESTÃO DE COBRANÇAS & RÉGUA DE COBRANÇA AUTOMÁTICA
   ========================================================================== */

/**
 * GET /api/billing
 * Lista todas as cobranças com suporte a filtros por empresa, status e busca
 */
apiRouter.get('/api/billing', requireAuth, (req: Request, res: Response) => {
  try {
    const { agentId, statusEnvio, statusPagamento, billingMethod, quickFilter, search } = req.query;

    const filter: BillingFilter = {
      agentId: agentId as string,
      statusEnvio: statusEnvio as any,
      statusPagamento: statusPagamento as any,
      billingMethod: billingMethod as any,
      quickFilter: quickFilter as any,
      search: search as string
    };

    // Atendentes com agente específico só enxergam cobranças daquela empresa
    if (req.user?.role === 'attendant' && req.user.assignedAgentId && req.user.assignedAgentId !== '*') {
      filter.agentId = req.user.assignedAgentId;
    }

    const charges = billingManager.getCharges(filter);
    res.json({ success: true, charges, total: charges.length });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
 ...
 * Retorna as métricas e indicadores analíticos (KPIs)
 */
apiRouter.get('/api/billing/stats', requireAuth, (req: Request, res: Response) => {
  try {
    let agentId = req.query.agentId as string;
    if (req.user?.role === 'attendant' && req.user.assignedAgentId && req.user.assignedAgentId !== '*') {
      agentId = req.user.assignedAgentId;
    }
    const stats = billingManager.getStats(agentId);
 ...
 */
apiRouter.post('/api/billing', requireAuth, async (req: Request, res: Response) => {
  try {
    const dto: CreateBillingDTO = req.body;

    // Se o atendente estiver vinculado a uma empresa, força a vinculação
    if (req.user?.role === 'attendant' && req.user.assignedAgentId && req.user.assignedAgentId !== '*') {
 ...
 * Retorna a configuração e status da Régua Diária
 */
apiRouter.get('/api/billing/scheduler/status', requireAuth, (_req: Request, res: Response) => {
  try {
    const config = billingScheduler.getConfig();
    const current = billingScheduler.getCurrentTimeAndDate(config.timezone);
    res.json({
      success: true,
      config,
 ...
 */
apiRouter.post('/api/billing/scheduler/config', requireAuth, (req: Request, res: Response) => {
  try {
    const updated = billingScheduler.updateConfig(req.body);
